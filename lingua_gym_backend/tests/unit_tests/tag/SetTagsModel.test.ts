...
describe('CardsTagsModel', () => {
    ...
    beforeEach(() => {
        ...
        setTagsModel = new SetTagsModel(mockDb as unknown as Database);

        mockSets = [
            { dictionarySetId: 'set1', name: 'Test Set', ownerId: 'user1', description: 'Test set', isPublic: false, createdAt: new Date() },
            { dictionarySetId: 'set2', name: 'Test Set 2', ownerId: 'user2', description: 'Test set 2', isPublic: false, createdAt: new Date() }
        ];
    ...
    test('addTagToSet - should return true when set is added', async () => {
        ...
        const result = await setTagsModel.addTagToSet(mockSets[0].dictionarySetId, mockTags[0].tagId);
    ...
    test('removeTagFromSet - should return true when tag is removed', async () => {
        ...
        const result = await setTagsModel.removeTagFromSet(mockSets[0].dictionarySetId, mockTags[0].tagId);
    ...
    test('getTagsForSet - should return list of tags', async () => {
        ...
        const result = await setTagsModel.getTagsForSet(mockSets[0].dictionarySetId);
